<div class="row">
  <div class="col-lg-12">
    <h3 class="page-header">Transaksi Mutasi Antar Gudang</h3>
    ...
    <ol class="breadcrumb">
      ...
      <li>Transaksi</li>
      <li>Edit Mutasi Antar Gudang</li>
    </ol>
  </div>  
</div>
  ...
  <div class="col-lg-12">
    <section class="panel">
      <div class="panel-body">
        ...
        <input type="hidden" name="ni_items" id="ni_items" value='<?php echo json_encode($new_tbm["items"]); ?>' required />
          <div class="form-group">
            <label class="col-sm-2 control-label">Tanggal </label>
            <div class="col-sm-3">
              <input tabindex="102" type="text" name="tanggal" id="tanggal" class="form-control" required="required" value="<?php echo isset($new_tbm['tanggal']) ? $new_tbm['tanggal']:''; ?>">
              <div class="help-block with-errors"></div>
            </div>
          </div>
          <div class="form-group">
            <label class="col-sm-2 control-label">Gudang pengambilan </label>
            <div class="col-sm-5">
              <select tabindex="102" type="search" name="gd_pengambilan" id="gd_pengambilan" class="form-control pull-left" required="required">
                <option value="">Pilih Gudang Pengambilan</option>
                <?php
                    foreach ($data_gudang as $row) {
                      if($new_tbm['gd_pengambilan']==$row->id_gudang){
          ...
          <div class="form-group">
            ...
            <div class="col-sm-5">
              <select tabindex="102" type="search" name="gd_tujuan" id="gd_tujuan" class="form-control pull-left" required="required">
                <option value="">Pilih Gudang Tujuan</option>
                <?php
                    foreach ($data_gudang as $row) {
                      if($new_tbm['gd_tujuan']==$row->id_gudang){
                        echo "<option value=".$row->id_gudang." selected>".$row->nama."</option>";
                      }else{
                        echo "<option value=".$row->id_gudang.">".$row->nama."</option>";
                      }
          ...
          <div class="form-group">
            ...
            <div class="col-sm-6">
              <textarea tabindex="103" class="form-control" rows="3" id="keterangan" name="keterangan"><?php echo isset($new_tbm['keterangan']) ? $new_tbm['keterangan']:''; ?></textarea>
              <div class="help-block with-errors"></div>
            </div>
          </div>
        <hr>
        ...
        <div class="form-group">
          ...
          <a keypress='alt b' tabindex="121" class="btn btn-danger" href="<?php echo base_url('transaksi_mutasi/reset'); ?>" notab><u>B</u>atal</a>
          </div>
        </div>
            <?php echo form_close(); //test($new_tbm); ?>
    </div>
    </section>
  </div>
</div>
...
APL.Brg = {
    ...
    $("#gd_pengambilan").select2().on('select2:select',function(e){});
    ...
    this.gd_pengambilan=$("#gd_pengambilan");
    ...
    _removefromgrid: function(el){
      ...
      $.get({
        url: APL.baseUrl+'transaksi_mutasi/remove_item',
        data: {
          index_code: code
        }
      });
    ...
    save: function(e){
      ...
      $.ajax({       
        data: { },     
        type : "POST",        
        url: APL.baseUrl+'transaksi_mutasi/edit_save',
        success : function(resp){
          ...
          if(resp.nomor_dok == 'ERROR INSERT' || resp.nomor_dok == false) {
            ...
            setTimeout(function () {
              window.location.href = APL.baseUrl+'transaksi_mutasi/'; //will redirect to google.
            }, 2000);
          }
        }
      })
